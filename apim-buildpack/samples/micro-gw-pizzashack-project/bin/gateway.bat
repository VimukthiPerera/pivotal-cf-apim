...
:callBallerina
	...
	if %verbose%==T echo [%date% %time%] DEBUG: balx location = "%GWHOME%\exec\pizzashack-project.balx"
	...
	IF %ERRORLEVEL% NEQ 0 (
		...
		%GWHOME%\runtime\bin\ballerina run "%GWHOME%\exec\pizzashack-project.balx" -e api.usage.data.path=%usage_data_path%  -e b7a.http.accesslog.path=%unix_style_path% --config "%GWHOME%\conf\micro-gw.conf" "%*" >> "%GWHOME%\logs\microgateway.log" 2>&1
	) else (
		...
		IF !PSVersion! LEQ 3 (
			...
			call powershell ".\runtime\bin\ballerina run .\exec\pizzashack-project.balx -e api.usage.data.path=$Env:usage_data_path  -e b7a.http.accesslog.path=$Env:unix_style_path --config .\conf\micro-gw.conf *>&1 | out-file -encoding ASCII -filepath .\logs\microgateway.log -Append"
		 ) else (
			...
			call powershell ".\runtime\bin\ballerina run .\exec\pizzashack-project.balx -e api.usage.data.path=$Env:usage_data_path -e b7a.http.accesslog.path=$Env:unix_style_path --config .\conf\micro-gw.conf *>&1 |  tee -Append .\logs\microgateway.log"
		)
	)
:end
goto endlocal
